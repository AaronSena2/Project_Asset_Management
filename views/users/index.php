...
<div class="card mb-4"><div class="card-body">
    ...
    <form method="post" action="/index.php?action=users.create">
        <div class="row g-3">
            ...
            <div class="col-md-3"><label class="form-label" for="new_user_password">Password</label><input id="new_user_password" class="form-control" type="password" name="password" autocomplete="new-password" minlength="8" pattern="(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[^A-Za-z0-9]).{8,}" title="Minimum 8 characters with uppercase, lowercase, number, and special character." required></div>
        ...
        <tr>
            ...
            <td>
                <form class="d-flex gap-2" method="post" action="/index.php?action=users.reset_password">
                    <input type="hidden" name="user_id" value="<?= (int) $userItem['id'] ?>">
                    <input aria-label="Set new password for user" class="form-control" type="password" name="new_password" autocomplete="new-password" minlength="8" pattern="(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[^A-Za-z0-9]).{8,}" title="Minimum 8 characters with uppercase, lowercase, number, and special character." required>
